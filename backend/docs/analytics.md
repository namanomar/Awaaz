...
Tracks every call. Identifies content gaps. Shows self-service rate and language distribution.

## Files

- `app/db.py` — SQLite schema + `log_call()` helper
- `app/logger.py` — called from the FastAPI webhook after every turn
...
Thin wrapper — import and call from `app/main.py`:
...
# ── Content gaps ──────────────────────────────────────────────────────────────
st.subheader("Content gaps — retrieval score below 0.60")
...
recent = df.head(100)[["created_at", "language", "intent", "query", "top_score", "escalated", "duration_s"]].copy()
...
Opens at `http://localhost:8501`. Auto-refreshes every 30 s.

---

## Deploying the dashboard (Streamlit Cloud — free)

1. Push the repo to GitHub
2. Go to [share.streamlit.io](https://share.streamlit.io) → New app
3. Point to `dashboard/dashboard.py`
...
> Note: SQLite doesn't work well on Streamlit Cloud (ephemeral filesystem). For production, switch `DATABASE_URL` to a Postgres connection string (e.g. Supabase free tier).

---

## Key metrics for the hackathon demo

| Metric | Target | Where |
|--------|--------|-------|
| Self-service rate | > 70% | KPI card |
| Avg retrieval score | > 0.72 | KPI card |
| Languages served | ≥ 3 | Language chart |
| Content gaps identified | Show the list | Gaps table |
| Top unanswered question | Call out 1 specifically | Gaps table |

The content gaps table is your strongest impact story — show that the system identifies its own blind spots and that adding new FAQ entries directly improves the self-service rate.
